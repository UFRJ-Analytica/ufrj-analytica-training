-- carga.sql — popula as tabelas finais a partir das tabelas brutas raw_
...
SELECT DISTINCT id_municipio, ano, valor, indicador, unidade, fonte
FROM raw_populacao_municipal
WHERE id_municipio IS NOT NULL
  AND ano IS NOT NULL
  AND id_municipio IN (SELECT id_municipio FROM municipios);
